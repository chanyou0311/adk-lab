...
タスク品質を維持できるか** を定量比較する実験 ([adk-lab](../../README.md) の実験のひとつ)。

同一の知識・タスク・モデルのまま、**知識の「置き場所」だけを変えた 6 バリアント**を作り、
16 タスク × N runs をクロスバリアントで走らせて、正答率 (Wilson 95% CI)・トークン・レイテンシ・
ツールルーティング精度・拒否率を決定的に採点する。

## 設計マトリクス (6 バリアント)

知識の中身は `src/lab/knowledge.py` の単一定義 (K1–K8)。バリアント間で変えるのは配置だけ。
...
- **support-sla**: K7 初回応答 SLA は pro=4h・それ以外 24h / K8 支払い関連 (決済/課金/返金) は plan に関わらず 4h
  — 3 つ目のユースケースとして後から追加し、「ユースケース追加時の各配置の増分コスト」の測定にも使う
  (`eval/results/STATIC_METRICS_UC3_DELTA.md`)

## 16 タスク (`eval/tasks.py`)
...
| F | SLA 違反件数 / 違反した pro チケット | 知識依存 (K7/K8 — 生データから導けない社内定義) |

ground truth は fixtures への DuckDB クエリで**採点時に計算** (ハードコードしない)。数値照合は
`answer_contains_number` (千/万/億・カンマ対応, 相対誤差 1%)、日付は数字境界付きで表記ゆらぎ吸収、
拒否は文単位 + 逆接ヘッジ判定の `is_refused` で検出する。

## モデル

Vertex AI の `gemini-3-flash-preview`。**global エンドポイント限定** (regional は 404) なので
`src/lab/model.py` の `_GlobalGemini` で api_client の location を global に固定する。
...
uv run python eval/run_eval.py                       # 全 6 variants × 16 tasks × runs=10
...
- `results{tag}_rescored.{json,md}` — 現行採点器でのオフライン再採点 (元ファイルは不変)
- `static_metrics.json` / `STATIC_METRICS.md` — バリアント別の固定コンテキストトークン量
- `STATIC_METRICS_UC3_DELTA.md` — ユースケース追加 (support-sla) による層別トークン増分
