...
他 5 バリアントを CLEAN/CONFUSABLE の 2 環境で回す計 13 セル。record に variant/env/cell/
tool_order_seed を残し、集計は cell (variant×env) 単位で行う。

Usage:
    # V-1 smoke (13 セル × 代表 3 タスク × 1 run)
    GOOGLE_CLOUD_PROJECT=<proj> uv run python eval/run_eval.py --smoke --tag _smoke
    # 本番 (13 セル × 16 タスク × runs=8)
...
# 収集セル計画: single_flat は 3 環境で劣化曲線を引き、他 5 バリアントは CLEAN と CONFUSABLE の
# 両端で比較する (計 13 セル)。DISTINCT の中間点は single_flat のみで測る。
_CELL_ENVS: dict[str, list[str]] = {
    ...
    "multi_taskmode": [CLEAN, CONFUSABLE],
...
def main() -> None:
    ...
    p.add_argument("--smoke", action="store_true",
                   help="V-1 smoke: 13 セル × 代表 3 タスク (A1/C1/E1) × 1 run に絞る")
