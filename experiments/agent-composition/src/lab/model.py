...
生成設定は temperature=1.0 + thinking_level=LOW で走らせる。gemini-3 系の thinking モデルは
temperature を下げると却って不安定になりうるため既定の 1.0 を使い、決定性は runs を増やして
統計 (Wilson CI) で吸収する。thinking は LOW に固定し、multi-agent 構成でも thinking 予算を
バリアント間で一定に保つ (thinking_budget / max_output_tokens は設定しない)。
"""
...
def make_generate_config() -> types.GenerateContentConfig:
    ...
    temperature=1.0 (gemini-3 thinking の既定) + thinking_level=LOW。thinking_budget と
    max_output_tokens は敢えて設定せず、thinking の粒度だけを LOW に固定する。
    """
